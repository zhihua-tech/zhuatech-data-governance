...
package cn.zhuatech.datagovernance.config;
import cn.zhuatech.datagovernance.domain.DomainCatalog;
import cn.zhuatech.datagovernance.model.*;
import cn.zhuatech.datagovernance.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "DGOV-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="DGOV-DEMO-"+String.format("%03d",sequence);
